...
function bottomUpSearch(a, b, c) {
  ...
  for (; f && f !== config.root.parentElement; ) {
    ...
    const h = index(f);
    if (b === Limit.All)
      h && (a = a.concat(a.filter(dispensableNth).map((a) => nthChild(a, h))));
    else if (b === Limit.Two)
      (a = a.slice(0, 1)),
        h &&
          (a = a.concat(a.filter(dispensableNth).map((a) => nthChild(a, h))));
    else if (b === Limit.One) {
...
function* combinations(a, b = []) {
  if (0 < a.length)
    for (let c of a[0]) yield* combinations(a.slice(1, a.length), b.concat(c));
  else yield b;
}
function sort(a) {
