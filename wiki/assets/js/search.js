...
(async function () {
    ...
    for (const [key, value] of Object.entries(content)) {
        //console.log(key);
        const cleanedContent = marked.parse(value.content, {renderer: renderMarkdownAsText()});
        contentIndex.add({
            id: key,
            title: value.title,
    ...
    const resultToHTML = ({url, title, content, term}) => {
        const text = marked.parse(content, {renderer: renderMarkdownAsText()});
